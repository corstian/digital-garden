...
The problems do not stop here. If the global state had changed in the meantime - meaning Blazor could not reconstruct the original page markup - this might very well mean that the original form could not be retrieved anymore. It's these framework level deficiencies which are incredibly difficult to work around in a generic manner, making it difficult to delitruly solid applications.
