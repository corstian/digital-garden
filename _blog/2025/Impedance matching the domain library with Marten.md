...
When it comes to registration of this projection with Marten, we can do so as follows:

```csharp
.AddMarten(options => { /* ... *})
    .AddProjectionWithServices<AggregateProjection<ShoppingList>>(ProjectionLifecycle.Inline, ServiceLifetime.Singleton)
```